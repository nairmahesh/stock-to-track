...
const CurrentOrders = () => {
  ...
  const fetchCurrentOrders = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .in("status", ["pending", "accepted", "dispatched"])
  ...
  return (
    <DealerLayout title="Current Orders">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Current Orders</h2>
            <p className="text-muted-foreground">Active orders in progress</p>
          </div>
          {orders.length > 0 && (
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground">All your orders are completed</p>
            </CardContent>
          </Card>
        ) : (
